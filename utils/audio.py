...
def logf0(wav_path, lower_f0=hparams.lower_f0, upper_f0=hparams.upper_f0):
    sr = hparams.sample_rate
    hop_len = int(sr * hparams.frame_shift_ms / 1000)
    ...
    subprocess.call('rm {}'.format(temp_raw_path), shell=True)
    subprocess.call('rm {}'.format(save_path), shell=True)
    return lf0s
...
def inv_mel_spectrogram(mel_spectrogram):
    S = _mel_to_linear(_db_to_amp(
        _denormalize(mel_spectrogram)+hparams.ref_level_db))
    return _griffin_lim(S ** hparams.power)
...
def _build_mel_basis():
    ...
    return librosa.filters.mel(
        ...
        fmin=hparams.min_mel_freq,
        fmax=hparams.max_mel_freq)
...
def _preemphasize(x):
    if len(x.shape) == 1:  # [time_steps]
        return signal.lfilter([1, -hparams.preemphasize], [1], x)
    elif len(x.shape) == 2:  # [batch_size, time_steps]
        if x.shape[0] == 1:
            return np.expand_dims(
                signal.lfilter([1, -hparams.preemphasize], [1], x[0]), axis=0)
        wav_list = list()
        for wav in x:
            wav_list.append(signal.lfilter([1, -hparams.preemphasize], [1], wav))
        return np.concatenate(wav_list, axis=0)
    else:
...
def inv_preemphasize(x):
    if hparams.preemphasize is None:
        return x
    if len(x.shape) == 1:  # [time_steps]
        return signal.lfilter([1], [1, -hparams.preemphasize], x)
    elif len(x.shape) == 2:  # [batch_size, time_steps]
        if x.shape[0] == 1:
            return np.expand_dims(
                signal.lfilter([1], [1, -hparams.preemphasize], x[0]), axis=0)
        wav_list = list()
        for wav in x:
            wav_list.append(signal.lfilter([1], [1, -hparams.preemphasize], wav))
        return np.concatenate(wav_list, axis=0)
    else:
...
def mfcc(y):
    ...
    mfcc = dct(x=S, axis=0, type=2, norm='ortho')[:hparams.n_mfcc]
