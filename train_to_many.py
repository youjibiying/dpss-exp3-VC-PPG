...
def main():
    ...
    test_dataloader = DataLoader(test_set, batch_size=hps.TrainToMany.test_batch_size,
                                 ...
                                 collate_fn=collate_fn)
    # set up model
    model = BLSTMToManyConversionModel(in_channels=hps.Audio.ppg_dim + 2,
    ...
    for epoch in range(hps.TrainToMany.epochs):
        ...
        torch.save(model.state_dict(), os.path.join(args.model_dir, "ppg-vc-to-many-{}.pt".format(epoch)))
        # validation
        print('Validating...')
        model.eval()
        dev_running_loss = 0.
